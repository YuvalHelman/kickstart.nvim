...
-- My Custom config:
vim.keymap.set('n', ';', ':')
-- Open new line without going in to insert-mode
vim.keymap.set('n', '<leader>o', 'o<esc>')
vim.keymap.set('n', '<leader>O', 'O<esc>')

--[[
Use <C-c> in macros when exiting insert mode. fixes a problem where <ESC> is
mapped to a different key in some terminals.
--]]
vim.keymap.set('i', '<esc>', '<C-c>')

--[[
replaces [x,d] to be DELETE instead of CUT. for cutting you now use <leader>
--]]
...
-- visual mode delete current selected and replace.
vim.keymap.set({ 'x' }, '<leader>p', '"_dP')

-- paste from insert mode
vim.keymap.set({ 'i' }, '<C-r><C-r>', '<C-r>*')

-- Movement in insert mode with the CTRL
vim.keymap.set({ 'i' }, '<C-l>', '<C-o>l')
vim.keymap.set({ 'i' }, '<C-j>', '<C-o>j')
vim.keymap.set({ 'i' }, '<C-k>', '<C-o>k')
vim.keymap.set({ 'i' }, '<C-h>', '<C-o>h')
-- <C-h> is problematic because it normally maps to BackSpace. not sure how
...
" File Tree Explorer
" -----------------------------------------------------------------------------
 Tweaks for browsing  - |netrw-browse-maps|
--]]
vim.g.netrw_banner = 0 --  disable annoying banner
vim.g.netrw_browse_split = 4 --  open in prior window
vim.g.netrw_altv = 1 --  open splits to the right
vim.g.netrw_liststyle = 3 --  tree view
vim.g.netrw_winsize = 15 --  Smaller Window
